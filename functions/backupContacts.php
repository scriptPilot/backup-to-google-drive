<?php
  ...
  function backupContacts($googleDriveService) {
    ...
    foreach ($contacts as $contact)  {
      $contactsIdents[] = $contact['resourceName'] . '/' . $contact['etag'] . '/v2';
    }
    ...
    foreach ($contacts as $contact) {
      ...
      if (!in_array($ident, $uniqueDriveIdents) && (isset($contact['names'][0]['displayName']) or isset($contact['emailAddresses'][0]['value']))) {
        $fileName = (isset($contact['names'][0]['displayName']) ? $contact['names'][0]['displayName'] : str_replace('@', ' ', $contact['emailAddresses'][0]['value'])) . '.vcf';
